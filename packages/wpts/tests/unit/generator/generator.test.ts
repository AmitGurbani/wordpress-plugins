    ...
    actions: [
      {
        ...
        body: {
          phpCode:
            "        load_plugin_textdomain( 'hello-greeter', false, 'hello-greeter/languages' );",
          sourceText: '',
        },
        context: 'public',
      },
    ...
    filters: [
      {
        ...
        body: {
          phpCode:
            "        $message = get_option( 'hello_greeter_greeting_message', 'Hello!' );\n        if ( is_single() ) {\n            return $content . '<div class=\"greeting\">' . esc_html( $message ) . '</div>';\n        }\n        return $content;",
          sourceText: '',
        },
        context: 'public',
      },
    ...
    shortcodes: [
      {
        ...
        body: {
          phpCode:
            "        $msg = isset( $atts['message'] ) ? $atts['message'] : get_option( 'hello_greeter_greeting_message', 'Hello!' );\n        return '<span class=\"greeting\">' . esc_html( $msg ) . '</span>';",
          sourceText: '',
        },
      },
    ],
...
describe('generatePlugin', () => {
  ...
  it('generates main class with hook registration', () => {
    ...
    const mainClass = files.find(
      (f) => f.relativePath === 'hello-greeter/includes/class-hello-greeter.php',
    )!;
  ...
  it('generates public class with hooks and shortcodes', () => {
    const ir = createTestIR();
    const files = generatePlugin(ir);
    const publicClass = files.find(
      (f) => f.relativePath === 'hello-greeter/public/class-hello-greeter-public.php',
    )!;
  ...
  it('generates activator with activation code', () => {
    const ir = createTestIR();
    const files = generatePlugin(ir);
    const activator = files.find(
      (f) => f.relativePath === 'hello-greeter/includes/class-hello-greeter-activator.php',
    )!;
    ...
    ir.ajaxHandlers = [
      {
        ...
        body: {
          phpCode: "\t\twp_send_json_success( array( 'deleted' => true ) );",
          sourceText: '',
        },
      },
    ];
