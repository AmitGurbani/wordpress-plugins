...
test.describe('Headless Meta Pixel — Diagnostics', () => {
  test('POST /diagnostics/test-capi requires authentication', async () => {
    ...
    const res = await ctx.post(
      `http://localhost:8889/wp-json/${SLUG}/v1/diagnostics/test-capi`,
    );
    expect(res.status()).toBe(401);
    await ctx.dispose();
  });

  test('POST /diagnostics/test-capi returns response structure', async ({
    restApi,
  }) => {
