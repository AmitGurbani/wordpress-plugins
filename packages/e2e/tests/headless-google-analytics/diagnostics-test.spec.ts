...
test.describe('Headless Google Analytics — Diagnostics', () => {
  test('POST /diagnostics/test-event requires authentication', async () => {
    ...
    const res = await ctx.post(
      `http://localhost:8889/wp-json/${SLUG}/v1/diagnostics/test-event`,
    );
    expect(res.status()).toBe(401);
    await ctx.dispose();
  });

  test('POST /diagnostics/test-event returns validation result', async ({
    restApi,
  }) => {
    ...
    const { status, data } = await restApi.post(
      `${SLUG}/v1/diagnostics/test-event`,
    );
