...
All endpoints are under `/headless-fuzzyfind/v1`.
...
The plugin is auto-mounted from `dist/headless-fuzzy-find/`. WooCommerce is auto-installed. Rebuild with `pnpm build` after code changes (or use `pnpm dev` in a separate terminal).
