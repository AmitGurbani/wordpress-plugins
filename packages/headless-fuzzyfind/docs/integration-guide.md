...
Headless FuzzyFind provides custom REST API endpoints for WooCommerce product search with weighted FULLTEXT matching, fuzzy correction, and autocomplete. This guide covers everything you need to integrate the search API into your storefront.

## Overview

Headless FuzzyFind provides two public REST endpoints:
...
**Base URL:** `https://your-site.com/wp-json/headless-fuzzyfind/v1`

If the site uses plain permalinks:
`https://your-site.com/?rest_route=/headless-fuzzyfind/v1`
...
```http
GET /wp-json/headless-fuzzyfind/v1/search?query=shirt&page=1&per_page=10&orderby=relevance
```

```bash
curl "https://your-site.com/wp-json/headless-fuzzyfind/v1/search?query=shirt&page=1&per_page=10"
```
...
```http
GET /wp-json/headless-fuzzyfind/v1/autocomplete?query=shirt&limit=5
```

```bash
curl "https://your-site.com/wp-json/headless-fuzzyfind/v1/autocomplete?query=shirt&limit=5"
```
...
input.addEventListener('input', function () {
  ...
  debounceTimer = setTimeout(() => {
    fetch(`/wp-json/headless-fuzzyfind/v1/autocomplete?query=${encodeURIComponent(query)}&limit=5`)
      .then(res => res.json())
      .then(data => {
...
For short words (under 3 characters, below MySQL's default `ft_min_word_len`), FULLTEXT may not match. Headless FuzzyFind automatically falls back to `LIKE` pattern matching to ensure results.

### Fuzzy Correction

When a search returns zero results, Headless FuzzyFind uses Levenshtein distance matching to correct misspelled words (edit distance ≤ 2) and retries the search with the corrected query.
