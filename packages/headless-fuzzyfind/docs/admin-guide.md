...
Headless FuzzyFind provides custom REST endpoints for search:

- **Search** — `GET /wp-json/headless-fuzzyfind/v1/search?query=...` (public, paginated, weighted relevance scoring)
- **Autocomplete** — `GET /wp-json/headless-fuzzyfind/v1/autocomplete?query=...` (public, lightweight suggestions for search-as-you-type)
...
1. **Check WooCommerce is active** — Headless FuzzyFind requires WooCommerce. Look for the admin notice
...
2. **Test with curl:**
   ```bash
   curl "https://your-site.com/wp-json/headless-fuzzyfind/v1/autocomplete?query=test"
   ```
