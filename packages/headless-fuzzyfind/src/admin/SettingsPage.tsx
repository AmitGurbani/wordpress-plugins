...
export function SettingsPage() {
  ...
  useEffect(() => {
    apiFetch({ path: '/headless-fuzzyfind/v1/settings' })
      .then((data: any) => {
        setSettings({ ...DEFAULTS, ...data });
        setLoading(false);
      })
      .catch(() => {
        setError(__('Failed to load settings.', 'headless-fuzzyfind'));
        setLoading(false);
      });
  }, []);
  ...
  const save = async () => {
    ...
    try {
      await apiFetch({
        path: '/headless-fuzzyfind/v1/settings',
        method: 'POST',
        data: settings,
      });
      setSaved(true);
      ...
      setError(__('Failed to save settings.', 'headless-fuzzyfind'));
    }
    setSaving(false);
  };
  ...
  return (
    <div className="wrap">
      <h1>{__('Headless FuzzyFind', 'headless-fuzzyfind')}</h1>
      ...
      <div style={{ marginTop: '16px' }}>
        <Button variant="primary" onClick={save} isBusy={saving}>
          {saving ? <Spinner /> : __('Save Settings', 'headless-fuzzyfind')}
        </Button>
        {saved && (
          <span style={{ marginLeft: '12px', color: '#00a32a' }}>
            {__('Settings saved.', 'headless-fuzzyfind')}
          </span>
        )}
        {error && (
