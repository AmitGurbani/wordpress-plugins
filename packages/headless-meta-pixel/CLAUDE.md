...
- **Sensitive settings**: `access_token` uses `sensitive: true` on @Setting — GET /settings returns `'********'` instead of the raw token; POST /settings preserves the real value when the UI re-submits the mask (admin saves without editing the token)
