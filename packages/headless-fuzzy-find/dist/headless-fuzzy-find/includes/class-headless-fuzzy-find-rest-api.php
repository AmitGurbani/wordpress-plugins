...
class Headless_Fuzzy_Find_Rest_Api {
	...
	public function search( $request ) {
		...
		$table_name = get_option( 'headless_fuzzy_find_index_table', '' );
		if ( ! $table_name ) {
			return rest_ensure_response( array( 'results' => array(), 'total' => 0, 'total_pages' => 0, 'page' => $page, 'per_page' => $per_page, 'did_you_mean' => array() ) );
		}
...
', $overrides_raw );
	...
	public function get_index_status( $request ) {
		...
		$last_indexed = get_option( 'headless_fuzzy_find_last_indexed', '' );
		$total_indexed = 0;
		if ( $table_name ) {
	...
	public function rebuild_index( $request ) {
		...
		update_option( 'headless_fuzzy_find_reindex_started', time() );
		$count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM %i WHERE post_type = \'product\' AND post_status = \'publish\'', $wpdb->posts ) );
		$total_products = $count ? intval( $count ) : 0;
		if ( $total_products <= 500 ) {
			do_action( 'headless_fuzzy_find_do_reindex' );
			return rest_ensure_response( array( 'message' => 'Reindex completed.', 'total_products' => $total_products ) );
		}
		$scheduled = wp_next_scheduled( 'headless_fuzzy_find_do_reindex' );
		if ( ! $scheduled ) {
			wp_schedule_single_event( time() + 5, 'headless_fuzzy_find_do_reindex' );
		}
	...
	public function delete_index( $request ) {
		...
		delete_option( 'headless_fuzzy_find_last_indexed' );
		delete_option( 'headless_fuzzy_find_reindex_in_progress' );
