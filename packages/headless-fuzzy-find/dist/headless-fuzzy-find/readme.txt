...
Headless Fuzzy Find replaces WooCommerce's default search with a FULLTEXT index-powered engine that understands relevance, handles typos, and returns results instantly via REST API.
...
3. Go to **Fuzzy Find** in the admin menu and click **Rebuild Index** to index your existing products.
...
No. Headless Fuzzy Find provides REST API endpoints (`/wp-json/headless-fuzzy-find/v1/search` and `/wp-json/headless-fuzzy-find/v1/autocomplete`) that you call from a custom search UI or headless frontend. The default WooCommerce search widget is unaffected.

= How do I build the search index for existing products? =

After activating, go to the **Fuzzy Find** admin page and click **Rebuild Index**. For stores with 500+ products, the rebuild runs via WP-Cron in the background (within a few seconds).
