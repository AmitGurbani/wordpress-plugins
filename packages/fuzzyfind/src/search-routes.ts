 ...
 *
 * GET /fuzzyfind/v1/search?query=...&page=1&per_page=10&orderby=relevance
...
class FfSearchRoutes {
  ...
  search(request: any): any {
    ...
    if (total === 0) {
      ...
      const threshold: number = intval(getOption('fuzzy_find_for_woo_commerce_did_you_mean_threshold', 3));
      if (threshold > 0) {
  ...
  autocomplete(request: any): any {
    ...
    const query: string = sanitizeTextField(request.get_param('query') ?? '');
    const rawLimit: number = intval(request.get_param('limit') ?? getOption('fuzzy_find_for_woo_commerce_autocomplete_limit', 8));
  ...
  buildBooleanQuery(searchTerm: string): string {
    const synonymsRaw: string = getOption('fuzzy_find_for_woo_commerce_synonyms', '');
