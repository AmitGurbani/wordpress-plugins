...
export function IndexTab() {
  ...
  const fetchStatus = () => {
    apiFetch({ path: '/fuzzyfind/v1/index/status' })
      .then((data: any) => {
        setStatus(data);
        setLoading(false);
      })
  ...
  const handleReindex = async () => {
    ...
    try {
      const result: any = await apiFetch({
        path: '/fuzzyfind/v1/index/rebuild',
        method: 'POST',
      });
      setMessage(result.message || __('Reindex started.', 'fuzzyfind'));
  ...
  return (
    <div style={{ padding: '16px 0', maxWidth: '600px' }}>
      <h3>{__('Index Status', 'fuzzyfind')}</h3>

      {status && (
        <table className="widefat" style={{ marginBottom: '16px' }}>
          <tbody>
            <tr>
              <td><strong>{__('Total Products', 'fuzzyfind')}</strong></td>
              <td>{status.total_products}</td>
            </tr>
            <tr>
              <td><strong>{__('Products Indexed', 'fuzzyfind')}</strong></td>
              <td>{status.total_indexed} ({percentage}%)</td>
            </tr>
            <tr>
              <td><strong>{__('Last Indexed', 'fuzzyfind')}</strong></td>
              <td>{lastIndexedDate}</td>
            </tr>
            <tr>
              <td><strong>{__('Status', 'fuzzyfind')}</strong></td>
              <td>{status.is_indexing ? __('Indexing in progress...', 'fuzzyfind') : __('Idle', 'fuzzyfind')}</td>
            </tr>
          </tbody>
        </table>
      )}
