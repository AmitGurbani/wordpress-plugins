...
class Fuzzyfind_For_Woocommerce_Rest_Api {
	...
	public function get_settings( $request ) {
		$settings = array(
			'weight_title' => get_option( 'fuzzy_find_for_woo_commerce_weight_title', 10 ),
			'weight_sku' => get_option( 'fuzzy_find_for_woo_commerce_weight_sku', 8 ),
			'weight_content' => get_option( 'fuzzy_find_for_woo_commerce_weight_content', 2 ),
			'fuzzy_enabled' => (bool) get_option( 'fuzzy_find_for_woo_commerce_fuzzy_enabled', true ),
			'autocomplete_enabled' => (bool) get_option( 'fuzzy_find_for_woo_commerce_autocomplete_enabled', true ),
			'analytics_enabled' => (bool) get_option( 'fuzzy_find_for_woo_commerce_analytics_enabled', true ),
			'min_query_length' => get_option( 'fuzzy_find_for_woo_commerce_min_query_length', 2 ),
			'autocomplete_limit' => get_option( 'fuzzy_find_for_woo_commerce_autocomplete_limit', 8 ),
			'did_you_mean_threshold' => get_option( 'fuzzy_find_for_woo_commerce_did_you_mean_threshold', 3 ),
			'synonyms' => get_option( 'fuzzy_find_for_woo_commerce_synonyms', '' ),
		);
	...
	public function update_settings( $request ) {
		...
		if ( isset( $params['weight_title'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_weight_title', $value );
		}
		if ( isset( $params['weight_sku'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_weight_sku', $value );
		}
		if ( isset( $params['weight_content'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_weight_content', $value );
		}
		if ( isset( $params['fuzzy_enabled'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_fuzzy_enabled', $value );
		}
		if ( isset( $params['autocomplete_enabled'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_autocomplete_enabled', $value );
		}
		if ( isset( $params['analytics_enabled'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_analytics_enabled', $value );
		}
		if ( isset( $params['min_query_length'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_min_query_length', $value );
		}
		if ( isset( $params['autocomplete_limit'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_autocomplete_limit', $value );
		}
		if ( isset( $params['did_you_mean_threshold'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_did_you_mean_threshold', $value );
		}
		if ( isset( $params['synonyms'] ) ) {
			...
			update_option( 'fuzzy_find_for_woo_commerce_synonyms', $value );
		}
	...
	public function search( $request ) {
		...
		$min_length = intval( get_option( 'fuzzy_find_for_woo_commerce_min_query_length', 2 ) );
		...
		$table_name = get_option( 'fuzzyfind_index_table', '' );
		if ( ! $table_name ) {
			return rest_ensure_response( array( 'results' => array(), 'total' => 0, 'total_pages' => 0, 'page' => $page, 'per_page' => $per_page, 'did_you_mean' => array() ) );
		}
		...
		$w_title = intval( get_option( 'fuzzy_find_for_woo_commerce_weight_title', 10 ) );
		$w_sku = intval( get_option( 'fuzzy_find_for_woo_commerce_weight_sku', 8 ) );
		$w_content = intval( get_option( 'fuzzy_find_for_woo_commerce_weight_content', 2 ) );
		...
		if ( $total === 0 ) {
			$threshold = intval( get_option( 'fuzzy_find_for_woo_commerce_did_you_mean_threshold', 3 ) );
			if ( $threshold > 0 ) {
	...
	public function autocomplete( $request ) {
		...
		$autocomplete_enabled = get_option( 'fuzzy_find_for_woo_commerce_autocomplete_enabled', '1' );
		if ( $autocomplete_enabled !== '1' ) {
			return new WP_Error( 'autocomplete_disabled', 'Autocomplete is disabled.', array( 'status' => 403 ) );
		}
		$query = sanitize_text_field( $request->get_param( 'query' ) ?? '' );
		$raw_limit = intval( $request->get_param( 'limit' ) ?? get_option( 'fuzzy_find_for_woo_commerce_autocomplete_limit', 8 ) );
	...
	public function get_index_status( $request ) {
		global $wpdb;
		$table_name = get_option( 'fuzzyfind_index_table', '' );
		$is_indexing = get_option( 'fuzzyfind_reindex_in_progress', '' );
		$last_indexed = get_option( 'fuzzyfind_last_indexed', '' );
		$total_indexed = 0;
		if ( $table_name ) {
	...
	public function rebuild_index( $request ) {
		...
		$is_indexing = get_option( 'fuzzyfind_reindex_in_progress', '' );
		if ( $is_indexing === '1' ) {
			$flag_time = intval( get_option( 'fuzzyfind_reindex_started', '0' ) );
			if ( $flag_time > 0 && (time() - $flag_time) < 600 ) {
				return new WP_Error( 'already_indexing', 'A reindex is already in progress.', array( 'status' => 409 ) );
			}
		}
		update_option( 'fuzzyfind_reindex_in_progress', '1' );
		update_option( 'fuzzyfind_reindex_started', time() );
		$count = $wpdb->get_var( $wpdb->prepare( 'SELECT COUNT(*) FROM %i WHERE post_type = \'product\' AND post_status = \'publish\'', $wpdb->posts ) );
		$total_products = $count ? intval( $count ) : 0;
		if ( $total_products <= 500 ) {
			do_action( 'fuzzyfind_do_reindex' );
			return rest_ensure_response( array( 'message' => 'Reindex completed.', 'total_products' => $total_products ) );
		}
		$scheduled = wp_next_scheduled( 'fuzzyfind_do_reindex' );
		if ( ! $scheduled ) {
			wp_schedule_single_event( time() + 5, 'fuzzyfind_do_reindex' );
		}
	...
	public function delete_index( $request ) {
		...
		delete_option( 'fuzzyfind_last_indexed' );
		delete_option( 'fuzzyfind_reindex_in_progress' );
		return rest_ensure_response( array( 'message' => 'Index cleared.' ) );
	}

	public function get_analytics( $request ) {
		global $wpdb;
		$log_table = get_option( 'fuzzyfind_log_table', '' );
		if ( ! $log_table ) {
	...
	public function correct_words( $search_term, $table_name ) {
		global $wpdb;
		$fuzzy_enabled = get_option( 'fuzzy_find_for_woo_commerce_fuzzy_enabled', '1' );
		if ( $fuzzy_enabled !== '1' ) {
	...
	public function build_boolean_query( $search_term ) {
		$synonyms_raw = get_option( 'fuzzy_find_for_woo_commerce_synonyms', '' );
...
', $synonyms_raw ) : array();
	...
	public function log_search( $search_term, $result_count ) {
		global $wpdb;
		$analytics_enabled = get_option( 'fuzzy_find_for_woo_commerce_analytics_enabled', '1' );
