...
class AuthRoutes {
  ...
  refreshToken(request: any): any {
    ...
    const storedHash: string = getUserMeta(userId, 'hoa_refresh_token_hash', true);
    ...
    const storedExpiry: number = intval(getUserMeta(userId, 'hoa_refresh_token_expiry', true));
    if (storedExpiry < time()) {
      deleteUserMeta(userId, 'hoa_refresh_token_hash');
      deleteUserMeta(userId, 'hoa_refresh_token_expiry');
      return new WP_Error('token_expired', 'Refresh token has expired.', { status: 401 });
    }
