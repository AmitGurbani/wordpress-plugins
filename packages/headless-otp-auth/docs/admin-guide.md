...
Access settings via the **OTP Auth** menu in WordPress admin. Settings are organized into 4 tabs.
...
2. If using plain permalinks: the `/wp-json/` path won't work. Use `?rest_route=/headless-otp-auth/v1/...` instead (e.g., `https://your-site.com/?rest_route=/headless-otp-auth/v1/otp/send`)
...
1. Ensure the plugin files are in `wp-content/plugins/headless-otp-auth/`
2. The main file should be `headless-otp-auth/headless-otp-auth.php`
