...
@Plugin({
  ...
  author: 'wpts',
  license: 'GPL-2.0+',
  textDomain: 'meta-pixel',
  requiresWP: '6.0',
  requiresPHP: '8.0',
})
@AdminPage({
  pageTitle: 'Meta Pixel Settings',
  menuTitle: 'Meta Pixel',
  capability: 'manage_options',
  menuSlug: 'meta-pixel-settings',
...
class MetaPixel {
  ...
  wooNotice(): void {
    if (!classExists('WooCommerce')) {
      ...
      echo(escHtml__('WooCommerce is recommended for automatic Purchase event tracking.', 'meta-pixel'));
      echo('</p></div>');
    }
  }
  ...
  @Filter('default_option_meta_pixel_currency', { priority: 11 })
  ...
  @Activate()
  onActivation(): void {
    updateOption('meta_pixel_version', '1.0.0');
  }
