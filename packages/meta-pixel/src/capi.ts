...
class MetaPixelCapi {
  ...
  onOrderStatusChanged(orderId: number, oldStatus: string, newStatus: string, order: any): void {
    ...
    if (getOption('meta_pixel_enable_purchase', '1') !== '1') {
      return;
    }
    ...
    const alreadySent: any = getPostMeta(orderId, '_meta_pixel_capi_sent', true);
    if (alreadySent) {
      return;
    }
    ...
    // Mark as sent
    updatePostMeta(orderId, '_meta_pixel_capi_sent', '1');
  }
}
