...
export function SettingsPage() {
  ...
  useEffect(() => {
    apiFetch({ path: '/meta-pixel/v1/settings' })
      .then((data: any) => {
        setSettings({ ...DEFAULTS, ...data });
        setLoading(false);
      })
      .catch(() => {
        setError(__('Failed to load settings.', 'meta-pixel'));
        setLoading(false);
      });
  }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      setError(__('Failed to save settings.', 'meta-pixel'));
    }
    setSaving(false);
  };
  ...
  return (
    <div className="wrap">
      <h1>{__('Meta Pixel Settings', 'meta-pixel')}</h1>
      ...
      <div style={{ marginTop: '16px' }}>
        <Button variant="primary" onClick={save} isBusy={saving}>
          {saving ? <Spinner /> : __('Save Settings', 'meta-pixel')}
        </Button>
        {saved && (
          <span style={{ marginLeft: '12px', color: '#00a32a' }}>
            {__('Settings saved.', 'meta-pixel')}
          </span>
        )}
        {error && (
