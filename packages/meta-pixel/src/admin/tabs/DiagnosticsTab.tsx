...
export function DiagnosticsTab() {
  ...
  const testCapi = async () => {
    ...
    try {
      const data = await apiFetch<TestResult>({
        path: '/meta-pixel/v1/diagnostics/test-capi',
        method: 'POST',
      });
      setResult(data);
    } catch (err: any) {
  ...
  const fetchLastError = async () => {
    ...
    try {
      const data = await apiFetch<{ last_error: string }>({
        path: '/meta-pixel/v1/diagnostics/last-error',
      });
  ...
  return (
    <div style={{ padding: '16px 0', maxWidth: '600px' }}>
      <h3>{__('Test Conversions API', 'meta-pixel')}</h3>
      <p style={{ color: '#666', marginBottom: '12px' }}>
        {__('Send a test PageView event to verify your CAPI connection. Make sure you have set a Test Event Code in the General tab first.', 'meta-pixel')}
      </p>
      <Button variant="secondary" onClick={testCapi} isBusy={testing}>
        {testing ? <Spinner /> : __('Test CAPI Connection', 'meta-pixel')}
      </Button>
      ...
      {result && (
        ...
        >
          ...
          {result.test_event_code && (
            <p>
              {__('Test Event Code:', 'meta-pixel')} <code>{result.test_event_code}</code>
            </p>
          )}
        </Notice>
      )}
      ...
      <h3>{__('Last CAPI Error', 'meta-pixel')}</h3>
      <Button variant="secondary" onClick={fetchLastError} isBusy={loadingError}>
        {loadingError ? <Spinner /> : __('Fetch Last Error', 'meta-pixel')}
      </Button>
