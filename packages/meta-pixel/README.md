...
Meta Pixel with WooCommerce integration and Conversions API (CAPI) for headless WordPress stores. The frontend handles browser-side pixel JS; this plugin handles server-side CAPI forwarding, settings, and automatic Purchase tracking via WooCommerce hooks.
...
The plugin is auto-mounted from `dist/meta-pixel/`. Rebuild with `pnpm build` after code changes (or use `pnpm dev` in a separate terminal).
...
All endpoints are under `/meta-pixel/v1/`.
...
Configured via WordPress admin page (Meta Pixel menu):
...
A `_meta_pixel_capi_sent` order meta flag prevents duplicate sends across status transitions.
