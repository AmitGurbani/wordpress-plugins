...
class Meta_Pixel_Rest_Api {
	...
	public function get_settings( $request ) {
		$settings = array(
			...
			'currency' => get_option( 'meta_pixel_currency', 'USD' ),
			'enable_view_content' => (bool) get_option( 'meta_pixel_enable_view_content', true ),
			'enable_add_to_cart' => (bool) get_option( 'meta_pixel_enable_add_to_cart', true ),
			'enable_initiate_checkout' => (bool) get_option( 'meta_pixel_enable_initiate_checkout', true ),
			'enable_purchase' => (bool) get_option( 'meta_pixel_enable_purchase', true ),
			'enable_search' => (bool) get_option( 'meta_pixel_enable_search', true ),
			'enable_capi' => (bool) get_option( 'meta_pixel_enable_capi', true ),
		);
	...
	public function update_settings( $request ) {
		...
		if ( isset( $params['pixel_id'] ) ) {
			...
			update_option( 'meta_pixel_pixel_id', $value );
		}
		if ( isset( $params['access_token'] ) ) {
			...
			update_option( 'meta_pixel_access_token', $value );
		}
		if ( isset( $params['test_event_code'] ) ) {
			...
			update_option( 'meta_pixel_test_event_code', $value );
		}
		if ( isset( $params['currency'] ) ) {
			...
			update_option( 'meta_pixel_currency', $value );
		}
		if ( isset( $params['enable_view_content'] ) ) {
			...
			update_option( 'meta_pixel_enable_view_content', $value );
		}
		if ( isset( $params['enable_add_to_cart'] ) ) {
			...
			update_option( 'meta_pixel_enable_add_to_cart', $value );
		}
		if ( isset( $params['enable_initiate_checkout'] ) ) {
			...
			update_option( 'meta_pixel_enable_initiate_checkout', $value );
		}
		if ( isset( $params['enable_purchase'] ) ) {
			...
			update_option( 'meta_pixel_enable_purchase', $value );
		}
		if ( isset( $params['enable_search'] ) ) {
			...
			update_option( 'meta_pixel_enable_search', $value );
		}
		if ( isset( $params['enable_capi'] ) ) {
			...
			update_option( 'meta_pixel_enable_capi', $value );
		}
	...
	public function track_event( $request ) {
		...
		if ( get_option( 'meta_pixel_' . $setting_key, '1' ) !== '1' ) {
	...
	public function test_capi( $request ) {
		...
		$payload = array( 'data' => array( array( 'event_name' => 'PageView', 'event_time' => time(), 'event_id' => 'test_' . strval( time() ), 'action_source' => 'website', 'event_source_url' => site_url(), 'user_data' => array( 'client_user_agent' => 'meta-pixel-plugin-test' ) ) ), 'test_event_code' => $test_event_code );
	...
	public function get_last_error( $request ) {
		return array( 'last_error' => get_option( 'meta_pixel_last_capi_error', '' ) );
	}
