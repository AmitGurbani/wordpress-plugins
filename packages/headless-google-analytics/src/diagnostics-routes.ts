...
class GoogleAnalyticsDiagnostics {
  ...
  testEvent(request: any): any {
    ...
    const url: string = 'https://www.google-analytics.com/debug/mp/collect?measurement_id=' + measurementId + '&api_secret=' + apiSecret;
    ...
    if (decoded && decoded['validationMessages'] !== undefined) {
      ...
      if (messages.length === 0) {
        return {
          success: true,
          message: 'Measurement Protocol validation passed. Note: the debug endpoint does not validate your Measurement ID or API Secret — verify events appear in GA4 Realtime.',
        };
      }
