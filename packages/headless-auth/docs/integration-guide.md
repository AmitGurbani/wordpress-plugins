...
OTP and password authentication with JWT tokens for headless applications. This guide covers everything you need to integrate the API into your mobile or web app.

## Overview

The API provides two authentication methods: phone-based OTP and username/email + password login. Both methods issue JWT access/refresh tokens for authenticated API access.

**Base URL:** `https://your-site.com/wp-json/headless-auth/v1`

If the site uses plain permalinks, the base URL is instead:
`https://your-site.com/?rest_route=/headless-auth/v1`
...
```bash
curl -X POST https://your-site.com/wp-json/headless-auth/v1/otp/send \
...
```bash
curl -X POST https://your-site.com/wp-json/headless-auth/v1/otp/verify \
  -H "Content-Type: application/json" \
...
Authenticate with username/email and password. Returns JWT tokens on success.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `username` | string | Yes | WordPress username or email address |
...
| Code | Status | When |
|------|--------|------|
| `missing_params` | 400 | Username or password not provided |
| `invalid_credentials` | 401 | Wrong username/email or password |
| `login_disabled` | 403 | Password login is disabled by site admin |
| `too_many_attempts` | 429 | Rate limit exceeded. Wait for the rate limit window to expire |
| `config_error` | 403 | JWT secret key is not configured on the server |
...
```bash
curl -X POST https://your-site.com/wp-json/headless-auth/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username": "john@example.com", "password": "mypassword"}'
```
...
```bash
curl -X POST https://your-site.com/wp-json/headless-auth/v1/auth/register \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://your-site.com/wp-json/headless-auth/v1/auth/refresh \
  -H "Content-Type: application/json" \
  -d '{"refresh_token": "eyJ..."}'
```
...
|------|--------|----------|-------|------------|
| `missing_params` | 400 | /auth/login | Username or password missing | Include both `username` and `password` fields |
| `invalid_credentials` | 401 | /auth/login | Wrong username/email or password | Check credentials. Generic error to prevent user enumeration |
| `login_disabled` | 403 | /auth/login | Password login disabled by admin | Contact site admin to enable password login |
| `too_many_attempts` | 429 | /auth/login | Too many failed login attempts | Wait for rate limit window to expire (default: 15 min) |
...
| **On expiry** | Use refresh token to get a new one | Re-authenticate with OTP or password |
...
| Limit | Default | Behavior |
|-------|---------|----------|
| Max login attempts | 5 per window | Blocked after 5 failed attempts for the same username/email |
| Rate limit window | 15 minutes | Login attempt counter resets after this period |
...
```javascript
const API_BASE = 'https://your-site.com/wp-json/headless-auth/v1';
