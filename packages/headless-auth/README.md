...
OTP and password authentication with JWT for headless WordPress stores. Supports phone-based OTP login, username/email + password login, and issues JWT access/refresh tokens.
...
The plugin is auto-mounted from `dist/headless-auth/`. Rebuild with `pnpm build` after code changes (or use `pnpm dev` in a separate terminal).
...
Configured via WordPress admin page (Auth menu):
...
| Enable Password Login | boolean | true | Allow username/email + password login |
| Max Login Attempts | number | 5 | Failed password login attempts before rate limiting |
