...
class Headless_Auth_Rest_Api {
	...
	public function get_settings( $request ) {
		$settings = array(
			'otp_test_mode' => (bool) get_option( 'headless_auth_otp_test_mode', false ),
			'otp_server_url' => get_option( 'headless_auth_otp_server_url', '' ),
			'otp_server_headers_template' => get_option( 'headless_auth_otp_server_headers_template', '{}' ),
			'otp_server_payload_template' => get_option( 'headless_auth_otp_server_payload_template', '{}' ),
			'otp_length' => get_option( 'headless_auth_otp_length', 6 ),
			'otp_expiry' => get_option( 'headless_auth_otp_expiry', 300 ),
			'max_otp_attempts' => get_option( 'headless_auth_max_otp_attempts', 3 ),
			'jwt_access_expiry' => get_option( 'headless_auth_jwt_access_expiry', 3600 ),
			'jwt_refresh_expiry' => get_option( 'headless_auth_jwt_refresh_expiry', 604800 ),
			'allowed_origins' => get_option( 'headless_auth_allowed_origins', '' ),
			'max_otp_verify_attempts' => get_option( 'headless_auth_max_otp_verify_attempts', 3 ),
			'otp_resend_cooldown' => get_option( 'headless_auth_otp_resend_cooldown', 60 ),
			'rate_limit_window' => get_option( 'headless_auth_rate_limit_window', 900 ),
			'enable_registration' => (bool) get_option( 'headless_auth_enable_registration', true ),
			'default_user_role' => get_option( 'headless_auth_default_user_role', 'subscriber' ),
			'enable_password_login' => (bool) get_option( 'headless_auth_enable_password_login', true ),
			'max_login_attempts' => get_option( 'headless_auth_max_login_attempts', 5 ),
		);
	...
	public function update_settings( $request ) {
		...
		if ( isset( $params['otp_test_mode'] ) ) {
			...
			update_option( 'headless_auth_otp_test_mode', $value );
		}
		...
		if ( isset( $params['otp_server_payload_template'] ) ) {
			...
			update_option( 'headless_auth_otp_server_payload_template', $value );
		}
		if ( isset( $params['otp_length'] ) ) {
			...
			update_option( 'headless_auth_otp_length', $value );
		}
		if ( isset( $params['otp_expiry'] ) ) {
			...
			update_option( 'headless_auth_otp_expiry', $value );
		}
		if ( isset( $params['max_otp_attempts'] ) ) {
			...
			update_option( 'headless_auth_max_otp_attempts', $value );
		}
		if ( isset( $params['jwt_access_expiry'] ) ) {
			...
			update_option( 'headless_auth_jwt_access_expiry', $value );
		}
		if ( isset( $params['jwt_refresh_expiry'] ) ) {
			...
			update_option( 'headless_auth_jwt_refresh_expiry', $value );
		}
		if ( isset( $params['allowed_origins'] ) ) {
			...
			update_option( 'headless_auth_allowed_origins', $value );
		}
		if ( isset( $params['max_otp_verify_attempts'] ) ) {
			...
			update_option( 'headless_auth_max_otp_verify_attempts', $value );
		}
		if ( isset( $params['otp_resend_cooldown'] ) ) {
			...
			update_option( 'headless_auth_otp_resend_cooldown', $value );
		}
		if ( isset( $params['rate_limit_window'] ) ) {
			...
			update_option( 'headless_auth_rate_limit_window', $value );
		}
		if ( isset( $params['enable_registration'] ) ) {
			...
			update_option( 'headless_auth_enable_registration', $value );
		}
		if ( isset( $params['default_user_role'] ) ) {
			...
			update_option( 'headless_auth_default_user_role', $value );
		}
		if ( isset( $params['enable_password_login'] ) ) {
			$value = rest_sanitize_boolean( $params['enable_password_login'] );
			if ( null === $value ) {
				return new \WP_Error(
			...
			update_option( 'headless_auth_enable_password_login', $value );
		}
		if ( isset( $params['max_login_attempts'] ) ) {
			$value = absint( $params['max_login_attempts'] );
			if ( null === $value ) {
				return new \WP_Error(
			...
			update_option( 'headless_auth_max_login_attempts', $value );
		}
	...
	public function send_otp( $request ) {
		...
		$otp_expiry = max( 60, intval( get_option( 'headless_auth_otp_expiry', 300 ) ) );
		$otp_hash = wp_hash_password( $otp );
		set_transient( 'headless_auth_otp_' . $phone_hash, $otp_hash, $otp_expiry );
		...
		$cooldown = max( 10, intval( get_option( 'headless_auth_otp_resend_cooldown', 60 ) ) );
		set_transient( $cooldown_key, strval( time() + $cooldown ), $cooldown );
		$test_mode = get_option( 'headless_auth_otp_test_mode', '' );
		if ( $test_mode === '1' ) {
			set_transient( 'headless_auth_test_otp_latest', wp_json_encode( array( 'otp' => $otp, 'phone' => $phone, 'created_at' => time() ) ), $otp_expiry );
		...
		$payload_template = get_option( 'headless_auth_otp_server_payload_template', '{}' );
		...
		$headers_template = get_option( 'headless_auth_otp_server_headers_template', '{}' );
	...
	public function verify_otp( $request ) {
		...
		delete_transient( 'headless_auth_verify_' . $phone_hash );
		...
		if ( count( $user_ids ) > 0 ) {
			...
			$access_token = apply_filters( 'headless_auth_generate_jwt', '', $existing_user_id, 'access', $access_expiry, $secret );
			$refresh_token = apply_filters( 'headless_auth_generate_jwt', '', $existing_user_id, 'refresh', $refresh_expiry, $secret );
			update_user_meta( $existing_user_id, 'headless_auth_refresh_token_hash', wp_hash_password( $refresh_token ) );
			update_user_meta( $existing_user_id, 'headless_auth_refresh_token_expiry', strval( time() + $refresh_expiry ) );
			delete_transient( 'headless_auth_refresh_grace_' . $existing_user_id );
		...
		set_transient( 'headless_auth_reg_' . $reg_token_hash, $phone, 600 );
	...
	public function get_test_otp( $request ) {
		...
		$data = get_transient( 'headless_auth_test_otp_latest' );
		if ( ! $data ) {
	...
	public function register_user( $request ) {
		...
		$access_expiry = intval( get_option( 'headless_auth_jwt_access_expiry', 3600 ) );
		$refresh_expiry = intval( get_option( 'headless_auth_jwt_refresh_expiry', 604800 ) );
		$access_token = apply_filters( 'headless_auth_generate_jwt', '', $user_id, 'access', $access_expiry, $secret );
		$refresh_token = apply_filters( 'headless_auth_generate_jwt', '', $user_id, 'refresh', $refresh_expiry, $secret );
		update_user_meta( $user_id, 'headless_auth_refresh_token_hash', wp_hash_password( $refresh_token ) );
		update_user_meta( $user_id, 'headless_auth_refresh_token_expiry', strval( time() + $refresh_expiry ) );
		delete_transient( 'headless_auth_refresh_grace_' . $user_id );
	...
	public function refresh_token( $request ) {
		...
		if ( ! $hash_matches ) {
			$grace_data = get_transient( 'headless_auth_refresh_grace_' . $user_id );
			if ( $grace_data ) {
		...
		$access_expiry = intval( get_option( 'headless_auth_jwt_access_expiry', 3600 ) );
		$refresh_expiry = intval( get_option( 'headless_auth_jwt_refresh_expiry', 604800 ) );
		$new_access_token = apply_filters( 'headless_auth_generate_jwt', '', $user_id, 'access', $access_expiry, $secret );
		$new_refresh_token = apply_filters( 'headless_auth_generate_jwt', '', $user_id, 'refresh', $refresh_expiry, $secret );
		update_user_meta( $user_id, 'headless_auth_refresh_token_hash', wp_hash_password( $new_refresh_token ) );
		update_user_meta( $user_id, 'headless_auth_refresh_token_expiry', strval( time() + $refresh_expiry ) );
		set_transient( 'headless_auth_refresh_grace_' . $user_id, wp_json_encode( array( 'prev_hash' => $stored_hash, 'access_token' => $new_access_token, 'refresh_token' => $new_refresh_token ) ), 30 );
	...
	public function get_profile( $request ) {
		...
		$caps = get_user_meta( $user_id, $cap_key, true );
		$roles = $caps ? array_keys( $caps ) : array();
		return array( 'id' => $user_id, 'first_name' => $first_name, 'last_name' => $last_name, 'name' => $display_name, 'email' => $email, 'phone' => $phone, 'roles' => $roles );
	}
	...
	public function update_profile( $request ) {
		...
		if ( $raw_last_name ) {
			$last_name = sanitize_text_field( $raw_last_name );
			if ( $last_name ) {
		...
		$display_name = get_the_author_meta( 'display_name', $user_id );
		$email_out = get_the_author_meta( 'user_email', $user_id );
		$first_name_out = get_the_author_meta( 'first_name', $user_id );
		$last_name_out = get_the_author_meta( 'last_name', $user_id );
		$phone = get_user_meta( $user_id, 'phone_number', true );
		$cap_key = $wpdb->prefix . 'capabilities';
		$caps = get_user_meta( $user_id, $cap_key, true );
		$roles = $caps ? array_keys( $caps ) : array();
		return array( 'id' => $user_id, 'first_name' => $first_name_out, 'last_name' => $last_name_out, 'name' => $display_name, 'email' => $email_out, 'phone' => $phone, 'roles' => $roles );
	}
	...
	public function login( $request ) {
		...
		if ( ! $user_id ) {
			$user_id = intval( email_exists( $login ) );
		}
		$secret = get_option( 'headless_auth_jwt_secret_key', '' );
		if ( ! $secret ) {
			return new WP_Error( 'config_error', 'JWT is not configured.', array( 'status' => 403 ) );
